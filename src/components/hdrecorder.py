...
class HDRecorderDialog(Gtk.Dialog):
    ...
    def update_rec_button(self, value=None):
        ...
        recorder = player.get_stream_recorder()
        if value == None:
            value = recorder.get_parameter_value(
                zzub.zzub_parameter_group_global, 0, 1)
    ...
    def update_label(self):
        player = com.get('neil.core.player')
        recorder = player.get_stream_recorder()
        self.btnsaveas.set_label(recorder.describe_value(
            zzub.zzub_parameter_group_global, 0, 0))
    ...
    def on_timer(self):
        ...
        if self.get_parent_window() and self.get_parent_window().is_visible():
            ...
            self.textposition.set_markup(
                "<b>Size</b> %.2fM" % (float(recsize)/(1 << 20)))
        return True
    ...
    def on_toggle_record(self, widget):
        ...
        player = com.get('neil.core.player')
        recorder = player.get_stream_recorder()
        recorder.set_parameter_value_direct(
            zzub.zzub_parameter_group_global, 0, 1, widget.get_active(), False)
