...
class SequencerToolBar(Gtk.HBox):
    ...
    def __init__(self, seqview):
        ...
        self.pack_start(self.steplabel, expand=False, fill=True, padding=0)
        self.pack_start(self.stepselect, expand=False, fill=True, padding=0)
        self.pack_start(self.followsong, expand=False, fill=True, padding=0)

    def increase_step(self):
        if self.get_parent().view.step < 64:
            self.get_parent().view.step *= 2
        self.update_stepselect()
        self.get_parent().update_all()

    def decrease_step(self):
        if self.get_parent().view.step > 1:
            self.get_parent().view.step /= 2
        self.update_stepselect()
        self.get_parent().update_all()
    ...
    def update_stepselect(self):
        ...
        try:
            self.stepselect.set_active(self.steps.index(self.get_parent().view.step))
            config.get_config().set_default_int('SequencerStep',
                                                self.get_parent().view.step)
            player.sequence_step = self.get_parent().view.step
        except ValueError:
            pass
        self.get_parent().view.adjust_scrollbars()
    ...
    def on_stepselect(self, widget, event=False):
        ...
        try:
            ...
            self.get_parent().view_step = 1
            return
        if widget.get_active() == -1 and event == False:
            return
        if self.get_parent().view.step == step:
            return
        if (step > 128):
            self.get_parent().view.step = 128
        if (step < 1):
            self.get_parent().view.step = 1
        else:
            self.get_parent().view.step = step
        self.get_parent().update_all()
        player = com.get('neil.core.player')
...
class SequencerPanel(Gtk.VBox):
    ...
    def __init__(self):
        ...
        tvkey.pack_start(cellkey, expand=True)
        tvpname.pack_start(cellpname, expand=True)
        ...
        self.pack_start(self.splitter, expand=True, fill=True, padding=0)
        self.pack_end(self.statusbar, expand=False, fill=True, padding=0)
    ...
    def on_sash_pos_changed(self, widget, *args):
        ...
        @type event: wx.Event
        """
        if not self.splitter.get_parent_window():
            return
        if not self.splitter.get_parent_window().is_visible():
            return
    ...
    def __set_properties(self):
        ...
        self.statusbar.pack_start(label, expand=False, fill=True, padding=0)
        self.statusbar.pack_start(Gtk.VSeparator(), expand=False, fill=True, padding=0)
        label = Gtk.Label()
        self.statuslabels.append(label)
        self.statusbar.pack_start(label, expand=False, fill=True, padding=0)
        self.statusbar.pack_start(Gtk.VSeparator(), expand=False, fill=True, padding=0)
