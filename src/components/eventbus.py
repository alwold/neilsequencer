...
EVENTS = [
    ...
    # (int, ... ). all connected objects should print out a random message.
    'ping',
    # (hub, ... ). all module are loaded and available, finalize any cross-dependencies.
    'finalize',
    # ( ... ). asks for quit. it is possible that quitting might not happen.
    'shutdown-request',
    # ( ... ). receiver should deinitialize and prepare for application exit.
    'shutdown',
    # ( ... ). called when the entire song changes in such a way that all connected objects should update.
    'song-opened',
    # (plugin, ...) called when a plugin should be visualized overall.
    'show-plugin',

    # most events serve for notification purposes, these are requests,
    # to be sent by a view to switch the workspace appearance.
    # (plugin, index, ...) called from e.g. the sequencer when editing a pattern is requested.
    'edit_pattern_request',
    # (plugin, index, time) called from e.g. the pattern editor, when editing a sequence is requested.
    ...
    # ([plugin, ...], ...) called when player.active_plugins changes.
    'active_plugins_changed',
    # ([(plugin, index), ...], ...) called when player.active_patterns changes.
    'active_patterns_changed',
    # ([wave, ...], ...) called when player.active_waves changes.
    'active_waves_changed',
    # (plugin, ...) called when player.autoconnect_target changes.
    'autoconnect_target_changed',
    # (stepsize, ...) called when player.sequence_step changes.
    'sequence_step_changed',
    # ((x,y), ...) called when player.plugin_origin changes.
    'plugin_origin_changed',
    # (plugin, ...) called when player.solo_plugin changes.
    'solo_plugin_changed',
    # (path, ...) called when player.document_path changes.
    ...
    'zzub_double_click',  # ( ... )
    # ( plugin,index,group,track,column,row,value,... )
    'zzub_edit_pattern',
    ...
    # ( plugin,index,row,rows,column_indices,indices,... )
    'zzub_pattern_insert_rows',
    # ( plugin,index,row,rows,column_indices,indices,... )
    'zzub_pattern_remove_rows',
    ...
    # ( plugin,... ) ## FIXME has been renamed in zzub as sequence.destroy
    'zzub_sequencer_remove_track',
...
class EventHandlerList:
    ...
    def filter_dead_references(self):
        ...
        """
        self.handlers = [(ref, funcname, args)
                         for ref, funcname, args in self.handlers if ref()]
