...
class TickDoublerDialog(Gtk.Dialog):
    ...
    def double_pattern(self, plugin, pattern_index):
        ...
        for r, g, t, i in self.pattern_range(plugin, pattern_index):
            ...
            pattern_contents.append(
                plugin.get_pattern_value(pattern_index, g, t, i, r))
            param = pl.get_parameter(g, i)
            plugin.set_pattern_value(
                pattern_index, g, t, i, r, param.get_value_none())
        item = 0
        plugin.set_pattern_length(
            pattern_index, plugin.get_pattern_length(pattern_index)*2)
        for r, g, t, i in pattern_indices:
            plugin.set_pattern_value(
                pattern_index, g, t, i, r*2, pattern_contents[item])
            item += 1
    ...
    def halve_pattern(self, plugin, pattern_index):
        ...
        for r, g, t, i in self.pattern_range(plugin, pattern_index):
            if r % 2:
                continue
            plugin.set_pattern_value(
                pattern_index, g, t, i, r/2, plugin.get_pattern_value(pattern_index, g, t, i, r))
    ...
    def pattern_range(self, plugin, pattern_index):
        ...
        group_track_count = [
            plugin.get_input_connection_count(),  1, plugin.get_track_count()]
        parameter_count = [plugin.get_pluginloader(
        ).get_parameter_count(group) for group in range(3)]
