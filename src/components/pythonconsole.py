...
VIEW_CLASS = Gtk.TextView.new_with_buffer
...
class PythonConsoleDialog(Gtk.Dialog):
    ...
    def __init__(self, hide_on_delete=True):
        ...
        view.modify_font(Pango.FontDescription(
            cfg.get_pattern_font('Monospace')))
        view.set_editable(False)
        view.set_wrap_mode(Gtk.WrapMode.WORD)
        self.consoleview = view
        self.buffer = buffer
        self.entry = Gtk.ComboBoxText.new_with_entry()
        self.entry.get_child().modify_font(
            Pango.FontDescription(cfg.get_pattern_font('Monospace')))
        renderer = self.entry.get_cells()[0]
        renderer.set_property(
            'font-desc', Pango.FontDescription(cfg.get_pattern_font('Monospace')))

        self.entry.get_child().connect('activate', self.on_entry_activate)
        self.textmark = self.buffer.create_mark(
...
class PythonConsoleMenuItem:
    ...
    def __init__(self, menu):
        # create a menu item
        item = Gtk.MenuItem.new_with_mnemonic(label="Show _Python Console")
